...
def cjpm_artifact_path(build_dir: Path, env, args=None) -> Path:
    ...
    return cjpm_profile_dir(build_dir, args) / "hotfixplugin" / ("libhotfixplugin" + suffix)
...
def build_plugin(project_dir: Path, build_dir: Path, args, env):
    ...
    if not (project_dir / "cjpm.toml").exists():
        print(f"cjpm module not found: {project_dir}")
        sys.exit(1)

    command = ["cjpm", "build", "-j", "4", "--target-dir", cjpm_target_dir(build_dir)]
    if args.build_type == "debug":
        command.append("-g")
    run_command(command, cwd=project_dir, env=env)
...
def run_unit_tests(project_dir: Path, build_dir: Path, env):
    ...
    test_result = run_command_result(command, cwd=project_dir, env=env)
