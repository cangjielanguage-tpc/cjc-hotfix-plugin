...
/*
  ...
  and guard class $Patch generated by plugin:
  ...
  this function generates patch class template:

  CHIR pseudocode:
  class @$PatchImpl <: @$Patch {
    [protected] [override] [final] func $fooPatch(%0: Class-$PatchImpl&) -> Unit {
      %1: Struct-_CNat6StringE = Constant("should not reach here")
      %2: Class-_CNat9ExceptionE& = Allocate(Class-_CNat9ExceptionE)
      %3: Unit = Apply(ThisType: Class-_CNat9ExceptionE&, @_CNat9Exception6<init>HRNat6StringE, %2, %1)
      RaiseException(%3)
    }
  }
 */
void Patcher::genPatchClass() const
{
    if (!guardClass) {
        return;
    }
    const auto patchName = PatchableName(guardClass).genPatchClassName();
    const auto patch = builder.CreateClass(INVALID_LOCATION, patchName, patchName, package->GetName(), true, false);
    const auto guardClassTypeImpl = builder.GetType<ClassType>(patch);
    patch->SetType(*guardClassTypeImpl);
    patch->SetSuperClassTy(*guardClass->GetType());
    patch->EnableAttr(Attribute::INTERNAL);
    patch->EnableAttr(Attribute::COMPILER_ADD);
    patch->Set<LinkTypeInfo>(Linkage::EXTERNAL);

    for (const auto& baseMethod : guardClass->GetMethods()) {
        const auto baseMethodType = baseMethod->GetFuncType();

        std::vector paramTypes = baseMethodType->GetParamTypes();
        paramTypes.front() = builder.GetType<RefType>(patch->GetType());
        const auto baseMethodReturnType = baseMethodType->GetReturnType();
        const auto patchMethodType = builder.GetType<FuncType>(paramTypes, baseMethodReturnType);

        const auto baseMethodName = baseMethod->GetSrcCodeIdentifier();
        const auto patchMethod = builder.CreateFunction(
            patchMethodType, baseMethodName, baseMethodName, baseMethodName, package->GetName(), {});
        patchMethod->EnableAttr(Attribute::OVERRIDE);
        patchMethod->EnableAttr(Attribute::PROTECTED);
        patchMethod->EnableAttr(Attribute::FINAL);
        patchMethod->Set<LinkTypeInfo>(Linkage::EXTERNAL);
        for (const auto paramType : paramTypes) {
            builder.CreateParameter(paramType, INVALID_LOCATION, *patchMethod);
        }

        const auto bg = builder.CreateBlockGroup(*patchMethod);
        patchMethod->InitBody(*bg);
...
Function* Patcher::createEmptyFunctionWithUnitRetVal(
    const std::string& name, const std::vector<Type*>& paramTypes) const
{
